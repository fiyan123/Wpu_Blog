...
<div class="card mb-3">
    ...
    @else
        <img src="https://source.unsplash.com/1200x450?{{ $posts[0]->category->name }}" class="card-img-top" alt="...">
    @endif
        ...
        <h3 class="card-title"><a href="/posts/{{ $posts[0]->slug }}" class="text-decoration-none text-dark">{{ $posts[0]->title }}</a></h3>
            <small class="text-muted">
                <p>By. <a href="/posts?author={{ $posts[0]->author->username }}" class="text-decoration-none">{{ $posts[0]->author->name }}</a> in
                    <a href="/posts?category={{ $posts[0]->category->slug }}" class="text-decoration-none">{{ $posts[0]->category->name }}</a> {{ $posts[0]->created_at->diffForHumans() }} {{-- UNTUK PENGAMBILAN WAKTU DIBUAT --}}
                </p>
            </small>
                        ...
                        <h5 class="card-title">{{ $post->title }}</h5>
                            <small class="text-muted">
                                <p>By. 
                                    ...
                                    {{ $post->created_at->diffForHumans() }} {{-- UNTUK PENGAMBILAN WAKTU DIBUAT --}}
                                </p>
                            </small>
